...
import 'package:e_shopee/exceptions/firebaseauth/reauth_exceptions.dart';
...
class AuthService {
  ...
  Future<bool> signUp({String? email, String? password, String? fName,String? lName, String? phoneNumber}) async {
    try {
      final UserCredential userCredential = await firebaseAuth
      ...
      await UserDatabaseHelper().createNewUser(uid, email, fName!,lName!, phoneNumber!);
      return true;
    } on MessagedFirebaseAuthException {
      rethrow;
    } on FirebaseAuthException catch (e) {
  ...
  Future<void> updateCurrentUserDisplayName(String fName, String lName) async {
  ...
  Future<bool> changeEmailForCurrentUser({String? password, String? newEmail}) async {
    try {
      if (password == null || newEmail == null) {
        throw FirebaseReauthException(); // or handle this case accordingly
      }

      bool isPasswordProvidedCorrect = await verifyCurrentUserPassword(password);
  ...
  Future<DocumentSnapshot?> fetchUserDocument(String uid) async {
    try {
